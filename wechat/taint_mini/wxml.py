...
"""
处理WXML文件。
:param file: WXML文件的路径。
:return: None
"""
def handle_alipay_wxml(file):
    try:
...
def handle_alipay_wxml_form(element):
    ...
    for e in element.iter():
        ...
        if e.tag == "g-form" or e.tag == "form":
            if hasattr(e, "attrib") and "onSubmit" in e.attrib:
                form_properties["bind_submit"] = e.attrib["onSubmit"]
                continue
...
def visit_wxml_tree(r):
    ...
    def visit_node(v):
        ...
        if hasattr(v, "tag") and (v.tag == "g-form" or v.tag == "form"):
            ...
            visited.extend(handle_alipay_wxml_form(v))
            return
