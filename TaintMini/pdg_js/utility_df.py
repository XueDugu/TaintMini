...
def limit_memory(maxsize):
    # 获取当前虚拟内存的限制
    limits = psutil.virtual_memory()
    
    # 设置虚拟内存的软限制和硬限制为 maxsize（单位为字节）
    limits = limits._replace(total=maxsize, available=min(maxsize, limits.available))
    psutil.virtual_memory = lambda: limits
