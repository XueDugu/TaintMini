...
def get_data_flow(input_file, benchmarks, store_pdgs=None, check_var=False, beautiful_print=False,
    ...
    if alt_json_path is not None:
        ...
        esprima_json = os.path.normpath(os.path.join(alt_json_path, esprima_json[1:]))
    ...
    if extended_ast is not None:
        ...
        if store_pdgs is not None:
            ...
            store_pdg = os.path.normpath(os.path.join(store_pdgs, os.path.basename(input_file.replace('.js', ''))))
            pickle_dump_process(dfg_nodes, store_pdg)
            json_analysis = os.path.normpath(os.path.join(store_pdgs, os.path.basename(esprima_json)))
...
def handle_one_pdg(root, js, store_pdgs):
    ...
    if js.endswith('.js'):
        ...
        js_path = os.path.join(root, js).replace('/', os.path.sep)
